...
//   const codeVerifier = generateRandomString(128);
//   localStorage.setItem(SESSION_STORAGE_PKCE_VERIFIER_KEY, codeVerifier);
//   const codeChallenge = await generateCodeChallenge(codeVerifier);
...
//   if (receivedState !== storedState) {
//     console.error('Invalid OAuth state');
//     throw new Error(`OAuth state mismatch. Potential CSRF attack. Stored state: ${storedState}, Received state: ${receivedState}`);
//   }
//   localStorage.removeItem('oauth_state');

//   const codeVerifier = localStorage.getItem(SESSION_STORAGE_PKCE_VERIFIER_KEY);
//   if (!codeVerifier) {
//     console.error('PKCE code verifier not found in storage.');
//     throw new Error('PKCE code verifier is missing. Please try logging in again.');
//   }
//   localStorage.removeItem(SESSION_STORAGE_PKCE_VERIFIER_KEY);
...
//     const data = await response.json();
//     if (data.access_token) {
//       localStorage.setItem(SESSION_STORAGE_GITHUB_TOKEN_KEY, data.access_token);
//       return data.access_token;
//     } else {
//       console.error('Access token not found in proxy response:', data);
//       throw new Error(`Access token not found in GitHub response via proxy. ${data.error_description || data.error || ''}`);
...
// const logout = () => {
//   localStorage.removeItem(SESSION_STORAGE_GITHUB_TOKEN_KEY);
//   localStorage.removeItem(SESSION_STORAGE_PKCE_VERIFIER_KEY);
//   localStorage.removeItem('oauth_state');
// };
...
// Define an interface for the service for clarity, though not applied directly to the const
interface IAuthService {
  loginWithGitHub: () => Promise<void>;
  handleGitHubCallback: (code: string, receivedState: string) => Promise<string | null>;
  logout: () => void;
}
...
const loginWithGitHub = async (): Promise<void> => {
  ...
  const state = generateRandomString(16);
  sessionStorage.setItem('oauth_state', state); // Use sessionStorage

  const codeVerifier = generateRandomString(128);
  sessionStorage.setItem(SESSION_STORAGE_PKCE_VERIFIER_KEY, codeVerifier); // Use sessionStorage
  const codeChallenge = await generateCodeChallenge(codeVerifier);
...
const handleGitHubCallback = async (code: string, receivedState: string): Promise<string | null> => {
  const storedState = sessionStorage.getItem('oauth_state'); // Use sessionStorage
  if (receivedState !== storedState) {
    console.error('Invalid OAuth state');
    throw new Error(`OAuth state mismatch. Potential CSRF attack. Stored state: ${storedState}, Received state: ${receivedState}`);
  }
  sessionStorage.removeItem('oauth_state'); // Use sessionStorage

  const codeVerifier = sessionStorage.getItem(SESSION_STORAGE_PKCE_VERIFIER_KEY); // Use sessionStorage
  if (!codeVerifier) {
    console.error('PKCE code verifier not found in session storage.');
    throw new Error('PKCE code verifier is missing. Please try logging in again.');
  }
  ...
  try {
    ...
    if (!response.ok) {
      let errorMessage = 'Unknown error during token exchange via proxy.';
      try {
        ...
        } else if (typeof errorData.message === 'string') { // For errors from the proxy itself
          errorMessage = errorData.message;
        } else if (errorData.error && typeof errorData.error.message === 'string') { // Nested error message
        ...
        // Failed to parse error JSON, use statusText or a generic message
        errorMessage = response.statusText || 'Failed to parse error response from proxy.';
        console.error('Token exchange via proxy failed:', response.status, 'Could not parse error JSON.');
      }
      throw new Error(`GitHub token exchange via proxy failed: ${errorMessage}. Ensure your backend proxy at ${proxyTokenUrl} is running and correctly configured.`);
    }

    const data = await response.json();
    if (data.access_token) {
      sessionStorage.setItem(SESSION_STORAGE_GITHUB_TOKEN_KEY, data.access_token); // Use sessionStorage
      return data.access_token;
    } else {
      console.error('Access token not found in proxy response:', data);
      const errorDetail = data.error_description || data.error || data.message || 'No access_token in response.';
      throw new Error(`Access token not found in GitHub response via proxy. ${errorDetail}`);
    }
  } catch (error) {
    console.error('Error during token exchange via proxy:', error);
    // Ensure the error message passed up is a string
    const message = (error instanceof Error) ? error.message : 'Network error or proxy issue. Ensure your backend proxy is reachable.';
    throw new Error(`Failed to exchange code for token via proxy. ${message}`);
  }
};

const logout = (): void => {
  sessionStorage.removeItem(SESSION_STORAGE_GITHUB_TOKEN_KEY); // Use sessionStorage
  sessionStorage.removeItem(SESSION_STORAGE_PKCE_VERIFIER_KEY); // Use sessionStorage
  sessionStorage.removeItem('oauth_state'); // Use sessionStorage
};
